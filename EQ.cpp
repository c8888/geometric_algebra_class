...
void EQ::calcError3(double dtmin, double dtmax, double OverallTime, double logarithm_base, Vec S0T, Rot R0T){
    if(logarithm_base<=1) __throw_invalid_argument("bad argument logarithm_base. Should be more than 1.");
    erase(); //THIS LINE IS OBLIGATORY!!!!
    double dt = dtmin;
    ...
    while(dt<dtmax){
        double diff = 0;
        calc(dt, int(OverallTime/dt), S0T, R0T);
        for(int i = 0; i < d.size(); i++){
            if( abs(1-d.getS(i).norm()) > diff ) diff = abs(1-d.getS(i).norm());
        }
...
void EQ::calcError4(double dtmin, double dtmax, double OverallTime, double logarithm_base, Vec S0T, Rot R0T){
    if(logarithm_base<=1) __throw_invalid_argument("bad argument logarithm_base. Should be more than 1.");
    double dt = dtmin;
    erase(); //THIS LINE IS OBLIGATORY!!!!
...
void EQ::calcError5(double dtmin, double dtmax, double OverallTime, double logarithm_base, Vec S0T, Rot R0T){
    if(logarithm_base<=1) __throw_invalid_argument("bad argument logarithm_base. Should be more than 1.");
    erase(); //THIS LINE IS OBLIGATORY!!!!
    double dt = dtmin;

    while(dt<dtmax){
        double diffTimesThetaMax = 0;
        calc(dt, int(OverallTime/dt), S0T, R0T);
        for(int i = 0; i < d.size(); i++){
            Rot Rexact = A->exactSol(d.getT(i), R0T); //getT(i) is the time at which getS(i) is specified
            Vec Sexact = Rexact * S0T * (~Rexact);
            double theta = acos((d.getS(i)<=Sexact)[0]/d.getS(i).norm()/Sexact.norm());
            double diff = abs(1-d.getS(i).norm());
            if( diff*theta > diffTimesThetaMax) diffTimesThetaMax = diff*theta;
        }
        push_back(pair<double, double> (log10(dt), log10(sqrt(diffTimesThetaMax))));
        dt *= logarithm_base;
    }
}
